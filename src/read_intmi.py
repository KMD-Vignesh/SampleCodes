    ...
    for testcase_folder in testcase_folders:
        ...
        for file_name in os.listdir(path=folder_path):
            if file_name.endswith('.csv'): 
                ...
                creation_time: float = os.path.getmtime(filename=file_path) 
        ...
        for file_name, file_path, _ in csv_files_with_ctime:
            ...
            csv_content: str = "<button style='margin-top:5px; margin-bottom:5px;' class='button button1' onclick='openPopup(`"+f"""{str(df.to_html(index=False, border=2, classes=["tableframe"])).replace("'",'"')}"""+"`)'>View CSV <i class='fa-regular fa-eye'></i></button>"
